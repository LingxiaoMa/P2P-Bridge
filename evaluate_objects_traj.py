...
from metrics.trajectory_metrics.straightness_metrics import TrajectoryMetrics
...
def parse_args():
    ...
    # ensure consistency config exists (for loading old checkpoints)
    if "consistency" not in cfg.diffusion:
        cfg.diffusion.consistency = omegaconf.OmegaConf.create(
            {"enabled": False, "lambda_cons": 0.1, "warmup_steps": 5000, "use_ema_target": True}
        )
...
def patch_based_denoise(
    ...
    Returns:
        Tuple[torch.Tensor, Optional[torch.Tensor], Dict[str, float]]:
            The denoised point cloud, the intermediate steps, and trajectory metrics.
    ...
    for b in range(B):
        traj = patches_steps[:, b, :, :]   # [T, n, 3]

        plr = TrajectoryMetrics.path_length_ratio(traj).mean().item()
        dc = TrajectoryMetrics.direction_consistency(traj).mean().item()

        traj_plr_list.append(plr)
        traj_dc_list.append(dc)

    mean_plr = sum(traj_plr_list) / len(traj_plr_list)
    mean_dc = sum(traj_dc_list) / len(traj_dc_list)

    traj_metrics = {
        "path_length_ratio": mean_plr,
        "direction_consistency": mean_dc,
    }
    ...
    return pcl_denoised, pcl_steps_denoised, traj_metrics
...
def sample(
    ...
    for res in resolutions:
        for noise in noises:
            ...
            # 新增：记录这个 setting 下所有 object 的 trajectory 指标
            traj_results = {}
            ...
            for data in input_iter(input_dir):
                ...
                with torch.no_grad():
                    model.eval()
                    pcl_next = pcl_noisy
                    pcl_next, pcl_next_steps, traj_metrics = patch_based_denoise(
                        model=model,
                        pcl_noisy=pcl_next,
                        patch_size=2048,
                        seed_k=cfg.k,
                ...
                # 新增：打印当前 object 的 trajectory 指标
                logger.info(
                    f"{data['name']} | "
                    f"path_length_ratio={traj_metrics['path_length_ratio']:.6f}, "
                    f"direction_consistency={traj_metrics['direction_consistency']:.6f}"
                )

                # 新增：保存当前 object 的 trajectory 指标
                traj_results[data["name"]] = {
                    "path_length_ratio": traj_metrics["path_length_ratio"],
                    "direction_consistency": traj_metrics["direction_consistency"],
                }
                ...
                if cfg.save_intermediate:
                    for step, item in enumerate(pcl_next_steps):
                        pcl_next_step = item.cpu()
                        pcl_next_step = pcl_next_step * data["scale"] + data["center"]
                        out_path = os.path.join(output_dir, "steps", data["name"], data["name"] + f"_{step}.xyz")
                        os.makedirs(os.path.dirname(out_path), exist_ok=True)
                        write_array_to_xyz(out_path, pcl_next_step.numpy())

            # 新增：把 trajectory 指标保存成 csv
            if len(traj_results) > 0:
                traj_df = pd.DataFrame(traj_results).transpose()
                traj_csv_path = os.path.join(output_dir, f"Summary_{cfg.dataset}_traj.csv")
                traj_df.to_csv(traj_csv_path, float_format="%.12f")

                traj_mean = traj_df.mean(axis=0)
                logger.info("\nTrajectory Results\n" + repr(traj_df))
                logger.info(
                    "\nTrajectory Mean\n"
                    + "\n".join([f"{k}\t{v:.12f}" for k, v in traj_mean.items()])
                )
